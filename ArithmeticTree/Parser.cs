...
namespace ArithmeticTree
{
    public static class Parser
    {
        ...
        static Func<IExpression, IExpression, IExpression> ParseOp(string s)
        {
            var t = ParseValue(s);
            IExpression Plus(IExpression x, IExpression y) => new BinaryExpression(x, y, "+");
            IExpression Minus(IExpression x, IExpression y) => new BinaryExpression(x, y, "-");
            IExpression Mult(IExpression x, IExpression y) => new BinaryExpression(x, y, "*");
            IExpression Div(IExpression x, IExpression y) => new BinaryExpression(x, y, "/");
            switch (t)
            {
                case Token.Plus:
                    return Plus;
        ...
        public static IExpression Evaluate(List<string> reversedRevPolNot, out int end, int start = 0)
        {
            ...
            switch (token)
            {
                ...
                case Token.Div:
                    int secondOpStart;
                    var rightOp = Evaluate(reversedRevPolNot, out secondOpStart, start + 1);
                    var leftOp = Evaluate(reversedRevPolNot, out end, secondOpStart);
                    var op = ParseOp(stocken);
                    return op(leftOp, rightOp);
                case Token.Number:
                    end = start + 1;
                    return new Literal(stocken);
            }
